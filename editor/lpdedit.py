...
from editor.pca_denoise.project import pca_project_denoise_by_threshold
from editor.pca_denoise.select_k import choose_k_from_singular_values
...
class LPDEDIT(BaseEditor):
    ...
    def predict_param_shifts(self) -> Dict[str, torch.FloatTensor]:
        ...
        for module_idx, module_name in enumerate(self.config.model.edit_modules):
            ...
            pca_cfg = self.config.editor.pca_denoise
            # PCA denoising
            if (pca_cfg.enable_pca):
                H_k = pca_project_denoise_by_threshold(hidden_states, pca_cfg)
                # Weighted averaging
                original_norm = hidden_states.norm(p=2)
                hidden_states = ( 1 - self.config.editor.alpha ) * hidden_states + self.config.editor.alpha * H_k
                hidden_states = hidden_states.to(device=hidden_states.device, dtype=hidden_states.dtype)
    ...
    def run(self, train_loader: DataLoader, valid_loader: DataLoader):
        """
        Use LPDEdit to finish sequential editing task.
        """
